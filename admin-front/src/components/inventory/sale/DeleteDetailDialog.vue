...
const emit = defineEmits(['update:isDialogVisible', 'deleteDetail'])
...
const distroy = async () => {
  ...
  try {
    const resp = await $api(`sale-details/${props.detailSelected.id}`, {
      method: 'DELETE',
    ...
    <VCard class="pa-sm-11 pa-3">
      ...
      <VCardText class="pt-5">
        ...
        <VForm class="mt-4" @submit.prevent="distroy">
          <VRow>
            <VCol cols="12">
              <p>Estas seguro de eliminar el detalle del producto: {{ props.detailSelected.product.title }}</p>
            </VCol>
