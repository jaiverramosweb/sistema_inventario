...
const emit = defineEmits(['update:isDialogVisible', 'refoundDeleteProduct'])
...
const distroy = async () => {
  ...
  try {
    ...
    emit('refoundDeleteProduct', props.refoundSelected)
    setTimeout(() => {
    ...
    <VCard class="pa-sm-11 pa-3">
      ...
      <VCardText class="pt-5">
        ...
        <VForm class="mt-4" @submit.prevent="distroy">
          <VRow>
            <VCol cols="12">
              <p>Estas seguro de eliminar el Incidencia del producto: {{ props.refoundSelected.product.title }}</p>
            </VCol>
