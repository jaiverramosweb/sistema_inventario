...
const emit = defineEmits(['update:isDialogVisible', 'deleteUnitComversion'])
...
const distroy = async () => {
  ...
  try {
    const resp = await $api(`unit-conversions/${props.unitSelected.id}`, {
      method: 'DELETE',
    ...
    emit('deleteUnitComversion', props.unitSelected)
    setTimeout(() => {
    ...
    <VCard class="pa-sm-11 pa-3">
      ...
      <VCardText class="pt-5">
        ...
        <VForm class="mt-4" @submit.prevent="distroy">
          <VRow>
            <VCol cols="12">
              <p>Estas seguro de eliminar la unidad de conversión: {{ props.unitSelected.unit_to }}</p>
            </VCol>
