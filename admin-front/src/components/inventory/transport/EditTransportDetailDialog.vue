...
const emit = defineEmits(["update:isDialogVisible", "editTransportDetail"]);
...
onMounted(() => {
  ...
  units.value = props.TransportDetailSelected.product.warehouses.filter(warehouse => warehouse.warehouse_id == props.warehause_start_id).map(wh => {
    return {
      id: wh.unit_id,
      name: wh.unit,
    }
...
const update = async () => {
  ...
  try {
    const resp = await $api(`transport-details/${props.TransportDetailSelected.id}`, {
      method: "PATCH",
      body: data,
...
const selectedUnit = () => {
  ...
  let price_celecte = props.TransportDetailSelected.product.wallets.find(
    (wallet) =>
  ...
  if (price_celecte) {
    ...
    let priceCelecte2 = props.TransportDetailSelected.product.wallets.find(
      (wallet) =>
        wallet.unit_id == unit_id.value &&
        wallet.type_client == type_client &&
    ...
    if (priceCelecte2) {
      ...
      price_unit.value =
        type_client == 1
          ? props.TransportDetailSelected.product.price_general
          : props.TransportDetailSelected.product.price_company;
    }
  }
}
