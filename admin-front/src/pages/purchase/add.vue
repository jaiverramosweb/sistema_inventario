...
const store = async () => {
  ...
  try {
    ...
    if(!warehouse_id.value){
      warning_purchase.value = 'Es necesario seleccionar un almacén'
      return
    }
    ...
    if(pushase_details.value.length == 0){
      warning_purchase.value = 'Es necesario agregar al menos un producto a la compra'
      return
    }
    ...
    const data = {
      ...
      pushase_details: pushase_details.value,
      total: total.value,
      importe: importe.value,
      iva: iva.value,
    }

    const resp = await $api('pushases', {
      method: 'POST',
      body: data,
      onResponseError({ response }) {
